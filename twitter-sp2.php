...
function sp2_get_custom_excerpt($post_id, $limit = 100) {
...
function sp2_get_text_to_send($post_id, $sp2_link = FALSE) {
	...
	switch ($text_to_send) {
		case 'post_title':
			...
			break;
		case 'post_excerpt':
			$return_text = sp2_get_custom_excerpt($post_id) . " " . $short_url;
			break;
		case 'title_excerpt':
			$text = $title . ': ';
			$limit = 110 - strlen($text);
			$text .= sp2_get_custom_excerpt($post_id, $limit);
			...
			3 chars "..." after the text - generated by sp2_get_custom_excerpt
			---
		...
		case 'custom':
			...
			if (strpos($text, "%fragment%") !== FALSE) {
				...
				$text = str_replace("%fragment%", sp2_get_custom_excerpt($post_id, $limit), $text);
